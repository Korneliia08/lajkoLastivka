      ...
      <div className={style.top}>
        <div className={style.container}>
          <div className={style.blockForImg}>
            {data.logo && data.logo.length > 3 ? (
              <img src={data.logo} alt="" className={style.imageStyle} />
            ) : (
              <img src={noImage} alt="" className={style.imageStyle} />
            )}
          </div>
          ...
          <div className={style.blockForDeleteAndToggle}>
            {data.isConnect ? (
              ...
              <MdOutlineWifiOff
                color={"rgba(0,25,138,0.71)"}
                title={
                  "Немає зв’язку з магазином. Підключіть його, щоб автоматизувати надсилання повідомлень"
                }
                size={20}
                className={style.wifiIc}
              />
            )}
            <MdDeleteOutline
